...
Tracker::Tracker(const double sigma_l, const double sigma_h,
                 const double sigma_iou, const int t_min)
    : sigma_l{sigma_l}, sigma_h{sigma_h}, sigma_iou{sigma_iou}, t_min{t_min},
      active_tracks{std::vector<Track>{}},
      finished_tracks{std::vector<Track>{}} {
...
void Tracker::track_iou_per_frame(std::vector<Detection> &detections) {
  ...
  for (int i = 0; i < this->active_tracks.size(); i++) {
    ...
    if (index != -1 &&
        intersectionOverUnion(track.boxes.back(), detections[index]) >=
            this->sigma_iou) {
      // If the box is good match for the track, then add it to the track.
      track.boxes.push_back(detections[index]);

      // If we found the max iou box of all time for the track, then update
      // track's max iou score
  ...
  for (auto box : detections) {
    ...
    b.push_back(box);
    // Track_id is set to 0 because we don't know if this track will "survive"
    // or not
...
//    const auto data = pub.get_message();
}
